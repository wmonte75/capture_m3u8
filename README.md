...
- `scp_enabled` / `scp_host` / `scp_username` / `scp_auth_type` / `scp_key_path` / `scp_password` / `scp_remote_movies_dir` / `scp_remote_tv_dir` / `scp_delete_local`: Optional SFTP remote transfer. Uploads finished downloads to a remote server (e.g., NAS) via SSH/SFTP after local processing completes.
