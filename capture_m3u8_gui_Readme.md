...
- **Remote SFTP Transfer**: Optional post-download upload to a remote server (NAS, VPS, etc.) via SSH key or password authentication.
...
4.  **Settings**: Update your library paths and speed limits directly in the UI; they save automatically. The **General** tab also contains an optional **Remote Transfer (SFTP)** section to push completed downloads to a remote server.
...
- Requires Python 3.8+
- Dependencies: `customtkinter`, `Pillow`, `requests`, `playwright`, `beautifulsoup4`, `paramiko`.
